...
public class CarAI : Car
{
    ...
    {
        desiredDirection = CustomDestination[destination].position - transform.position;
        if (desiredDirection.magnitude <= reachingRadius)
        {
            destination++;
            if (destination >= CustomDestination.Length)
            {
                destination = 0;
            }
        }
    ...
    {
        Ray lookingRay = new Ray(transform.position, lookingDirection);
    ...
    private void AvoidObstacle()
    {
        ...
        for (int i = 0; i < 36; i++)
        {
            lookingDirection = Quaternion.Euler(0, i * 20f, 0) * PostionToFollow;
            if (!IsBlocked(lookingDirection))
            {
                PostionToFollow = lookingDirection;
                break;
            }
        }
    }
    ...
    private void PathProgress() //Checks if the agent has reached the currentWayPoint or not. If yes, it will assign the next waypoint as the currentWayPoint depending on the input
    {
        ...
        void wayPointManager()
        {
            ...
            else
            {
                ...
                allowMovement = true;
                if (Vector3.Distance(carFront.position, PostionToFollow) < 25)
                    currentWayPoint++;
            }
        ...
        void CreatePath()
        {
            if (CustomDestination == null)
            {
               ...
               CustomPath(CustomDestination[destination]);
    ...
    void ApplySteering() // Applies steering to the Current waypoint
    {
        ...
        if (SteeringAngle > 15) LocalMaxSpeed = 200;
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.tag == "CheckPoint")
        {
            ...
            if (checkPoint > 2)
                carAi.checkPointId++;
            if (checkPoint == CustomDestination.Length)
            {
                carAi.lap++;
